                        ...
                        <section class="pt-6 pb-20 mx-8 w-auth">
                            <div class="grid gap-5 md:grid-cols-12">
                                        ...
                                        <div class="flex overflow-x-scroll hide-scroll-bar dragscroll">
                                            <div class="flex mt-2 flex-nowrap">
                                                @forelse ($thumbnail as $item)
                                                    <img src="{{ url(Storage::url($item->thumbnail)) }}" alt="Thumbnail Service" :class="{ 'border-4 border-serv-button': active === {{ $item->id }} }" @click="changeThumbnail(' {{ url(Storage::url($item->thumbnail)) }} ', {{ $item->id }} )" class="inline-block w-24 mr-2 rounded-lg cursor-pointer">
                                                @empty
                                                    {{-- empty --}}
                                                @endforelse
                                            </div>
                                ...
                                <div class="p-4 md:text-right lg:col-span-6 md:col-span-12">
                                    ...
                                    <a href="{{ route('service.edit', $service->id) }}" class="inline-flex justify-center px-4 py-2 text-sm font-medium text-white border border-transparent rounded-lg shadow-sm bg-serv-email hover:bg-serv-email-text focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-serv-email">
                                        Edit Service
                                    </a>
                                </div>
