                                    ...
                                    <tr class="text-gray-700 border-b">
                                            ...
                                            <div class="flex items-center text-sm">
                                                <div class="relative w-10 h-10 mr-3 rounded-full md:block">
                                                    @if ($order->user_buyer->detail_user->photo != null || '')
                                                        <img class="object-cover w-full h-full rounded-full" alt="photo freelancer" loading="lazy" src="{{ url(Storage::url($order->user_buyer->detail_user->photo)) }}"/>
                                                    @else
                                        ...
                                        <td class="px-1 py-5 text-sm">
                                            @if ($order->order_status_id == '1')
                                                <a href="{{ route('order.show', $order->id) }}" class="px-4 py-2 mt-1 mr-2 text-center text-white rounded-xl bg-serv-email">Details</a>
                                            ...
                                            @elseif($order->order_status_id == '2')
                                                <a href="{{ route('order.show', $order->id) }}" class="px-4 py-2 mt-2 mr-2 text-center text-white rounded-xl bg-serv-email">Details</a>

                                                <a href="{{ route('order.edit', $order->id) }}" class="px-4 py-2 mt-2 text-center text-white rounded-xl bg-serv-email">Submit</a>
                                            ...
                                            @elseif ($order->order_status_id == '4')
                                                <a href="{{ route('accept.order', $order->id) }}" class="px-4 py-2 mt-2 text-left text-white rounded-xl bg-serv-button">Accept</a>

                                                <a href="{{ route('reject.order', $order->id) }}" class="px-4 py-2 text-left bg-white rounded-xl">Reject</a>
                                                </p>
