                                        ...
                                        <div class="flex items-center text-sm">
                                            <div class="relative w-10 h-10 mr-3 rounded-full md:block">
                                                @if ($order->user_freelancer->detail_user->photo != null || '')
                                                    <img class="object-cover w-full h-full rounded-full" alt="photo freelancer" loading="lazy" src="{{ url(Storage::url($order->user_freelancer->detail_user->photo)) }}"/>
                                                @else
                                                    <svg class="object-cover w-full h-full rounded-full" fill="currentColor" viewBox="0 0 24 24">
                                    ...
                                    <td class="w-2/6 px-1 py-5">
                                        <div class="flex items-center text-sm">
                                            <div class="relative w-10 h-10 mr-3 rounded-full md:block">
                                                ...
                                                @if ($order->service->thumbnail_services[0]->thumbnail != null || '')
                                                    <img class="object-cover w-full h-full rounded-full" alt="photo freelancer" loading="lazy" src="{{ url(Storage::url($order->service->thumbnail_services[0]->thumbnail)) }}"/>
                                                @else
                                                    <svg class="object-cover w-full h-full rounded-full" fill="currentColor" viewBox="0 0 24 24">
                                                        <path d="M24 20.993V24H0v-2.996A14.977 14.977 0 0112.004 15c4.904 0 9.26 2.354 11.996 5.993zM16.002 8.999a4 4 0 11-8 0 4 4 0 018 0z" />
                                                    </svg>
                        ...
                        <form action="#" method="POST">
                            <div class="">
                                ...
                                <div class="px-1 py-4 text-right">
                                    <a type="button" href="{{ route('request.index') }}" class="inline-flex justify-center px-4 py-2 mr-4 text-sm font-medium text-gray-700 bg-white border border-gray-600 rounded-lg shadow-sm hover:bg-white focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-300" onclick="return confirm('Are you sure want to back ? any changes you make will not be saved')">
                                        Back
                                    </a>
                                    
                                    @if ($order->order_status_id == 2 && isset($order->file))
                                        
                                        <a type="button" href="{{ route('approve.requrest', $order->id) }}" class="inline-flex justify-center px-4 py-2 text-sm font-medium text-white bg-green-600 border border-transparent rounded-lg shadow-sm hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500" onclick="return confirm('Are you sure want to approve this data ?')">
                                            Approve
                                        </a>
                                    @else
                                        <a type="button"  class="inline-flex justify-center px-4 py-2 text-sm font-medium text-white bg-green-600 border border-transparent rounded-lg shadow-sm hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500" @disabled(true)>
                                            Can't Approve
                                        </a>
