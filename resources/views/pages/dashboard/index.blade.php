            ...
            <div class="grid w-full gap-5 px-10 mx-auto md:grid-cols-12">
                ...
                <div class="col-span-4 text-right">
                    <div @click.away="open = false" class="relative z-10 hidden mt-5 lg:block" x-data="{ open: false }">
                        ...
                        @else
                            <svg class="inline w-12 h-12 mr-3 rounded-full" fill="currentColor" viewBox="0 0 24 24">
                                            ...
                                            <div class="flex items-center text-xs">
                                                <div class="relative w-8 h-8 mr-3 rounded-full md:block">
                                                    ...
                                                    @if ($item->user_buyer->detail_user->photo != null || '')
                                                        <img src="{{ url(Storage::url($item->user_buyer->detail_user->photo)) }}" alt="photo profile" class="object-cover w-8 h-8 mr-3 rounded-full">
                                                    @else
                                                        <svg class="object-cover w-8 h-8 mr-3 rounded-full" fill="currentColor" viewBox="0 0 24 24">
                                                            <path d="M24 20.993V24H0v-2.996A14.977 14.977 0 0112.004 15c4.904 0 9.26 2.354 11.996 5.993zM16.002 8.999a4 4 0 11-8 0 4 4 0 018 0z" />
                                                        </svg>
                                                    @endif
                                                ...
                                                <div>
                                                    <p class="text-xs text-black">{{ $item->user_buyer->name ?? '' }}</p>
                                                    
                                                    @if ($item->order_status_id == '1')
                                                        <p class="text-xs text-green-500">{{ $item->order_status->name ?? '' }}</p>
                                                    @elseif ($item->order_status_id == '2')
                                                        <p class="text-xs text-yellow-500">{{ $item->order_status->name ?? '' }}</p>
                                                    @elseif ($item->order_status_id == '3')
                                                        <p class="text-xs text-red-500">{{ $item->order_status->name ?? '' }}</p>
                                                    @else
                                                        <p class="text-xs text-black-500">{{ $item->order_status->name ?? '' }}</p>
                                                    @endif
                                                </div>
                                            </div>
                                        </td>
                                        ...
                                        <td class="w-2/4 px-1 py-5">
                                            <div class="flex items-center text-xs">
                                                <div class="relative w-10 h-10 mr-3 rounded-full md:block">
                                                    @if (count($item->service->thumbnail_services))
                                                        @if($item->service->thumbnail_services[0]->thumbnail != null)
                                                            <img class="object-cover h-8 w-8 rounded" src="{{ url(Storage::url($item->service->thumbnail_services[0]->thumbnail)) }}" alt="" loading="lazy" />
                                                        @else
                                                            <svg class="object-cover h-8 w-8 rounded text-gray-300" fill="currentColor" viewBox="0 0 24 24">
                                                                <path d="M24 20.993V24H0v-2.996A14.977 14.977 0 0112.004 15c4.904 0 9.26 2.354 11.996 5.993zM16.002 8.999a4 4 0 11-8 0 4 4 0 018 0z" />
                                                            </svg>
                                                        @endif
                                                    @else
